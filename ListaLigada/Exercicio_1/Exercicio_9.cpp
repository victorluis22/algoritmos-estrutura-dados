...
class intSLLNodeList{
	...
	public:
		...
		void reverseIntSLLNodeList() {
			intSLLNodeList sl2;
			intSLLNode *tmp;
			
			for(tmp = head; tmp != 0; tmp = tmp->next){
				sl2.addToHead(tmp->info);
			}
			
			head = sl2.head;
		}
		
		intSLLNodeList* separate(intSLLNodeList* l, int n){
			intSLLNode *tmp;
			intSLLNodeList *sl2;
			static intSLLNodeList sl3;
			
			sl2 = &sl3;
			
			
			for(tmp = head; tmp->info != n && tmp != 0; tmp = tmp->next){
				l->addToTail(tmp->info);
			}
